...
export async function GET(request, { params }) {
  try {
    ...
    // NUEVO: igual que Escalas, Eliminar depende únicamente del permiso
    // de la matriz (POST_VUELO.puede_eliminar) — sin extenderlo a "o
    ...
    console.error("Error GET post-vuelo:", error)
    return NextResponse.json({ error: "Error interno al obtener el post-vuelo" }, { status: 500 })
  }
}
...
export async function POST(request, { params }) {
  try {
    ...
    console.error("Error POST post-vuelo:", error)
    return NextResponse.json({ error: "Error interno al crear el post-vuelo" }, { status: 500 })
  }
}
...
export async function PUT(request, { params }) {
  try {
    ...
    console.error("Error PUT post-vuelo:", error)
    return NextResponse.json({ error: "Error interno al editar el post-vuelo" }, { status: 500 })
  }
}
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!session.user.permisos?.POST_VUELO?.puede_eliminar) {
      return NextResponse.json({ error: "No tenés permiso para eliminar un post-vuelo" }, { status: 403 })
    }
    ...
    console.error("Error DELETE post-vuelo:", error)
    return NextResponse.json({ error: "Error interno al eliminar el post-vuelo" }, { status: 500 })
  }
}